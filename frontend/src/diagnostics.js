...
function describe(stage, d) {
  const stages = {
    ...
    queued: [d.position ? `Queued · ${d.position} ahead` : 'Generation slot acquired', 'One active generation is allowed. Queue positions update with server heartbeats.'],
    ...
    agent_round: [`Agent round ${d.round}`, `${d.tool_calls}/${d.max_tool_calls} tool calls used. The model is deciding whether to execute a tool or answer.`],
    ...
    sandbox_stopped: ['E2B sandbox terminated', `Sandbox ${d.sandbox_id} is stopped. Saved files remain available in this conversation.`],
